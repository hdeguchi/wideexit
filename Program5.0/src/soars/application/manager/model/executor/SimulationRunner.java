 ...
 *
 */
public class SimulationRunner extends Executor {
	...
	public static boolean start(File file) {
		...
		if ( 0 <= osname.indexOf( "Windows")) {
			...
			//list.add( "-D" + Constant._macScreenMenuName + "=SOARS VisualShell");
		...
		list.add( "-jar");
		list.add( homeDirectoryName + "/" + ( is_visualShell4( file) ? Constant._visualShell4JarFilename : Constant._visualShellJarFilename));
		list.add( "-language");
