...
public class ExTransferTable extends VariableTableBase implements DropTargetListener {
	 ...
	 * @param parent
	 */
	public ExTransferTable(EntityBase entityBase, Map<String, PropertyPanelBase> propertyPanelBaseMap, PropertyPanelBase propertyPanelBase, Frame owner, Component parent) {
		super("extransfer", entityBase, propertyPanelBaseMap, propertyPanelBase, 3, owner, parent);
	}
	...
	public String[] get_agent_file_names(boolean containsEmpty) {
		...
		return LayerManager.get_instance().get_agent_object_names( "extransfer", filenames, _entityBase, containsEmpty);
	}
...
//		return LayerManager.get_instance().move_file( srcPath, destPath);
	...
	public boolean setup() {
		...
		if ( !_entityBase.is_multi()) {
			if ( !setup( _entityBase._objectMapMap.get( "extransfer"), defaultTableModel))
				return false;
		}
	...
	private void update_initial_value(int row, File file) {
		...
		( ( ExTransferPropertyPanel)_propertyPanelBase).changeSelection( ( ExTransferObject)getValueAt( row, 0), true);
	}
	...
	@Override
	public void on_ok() {
		_entityBase._objectMapMap.get( "extransfer").clear();

		for ( int i = 0; i < getRowCount(); ++i) {
			ExTransferObject exTransferObject = ( ExTransferObject)getValueAt( i, 0);
			_entityBase._objectMapMap.get( "extransfer").put( exTransferObject._name, exTransferObject);
		}
	}
    ...
    DataFlavor[] dataFlavors = arg0.getCurrentDataFlavors();
		if ( dataFlavors[ 0].getHumanPresentableName().equals( DirectoryNodeTransferable._name)
			...
			try {
				...
				setRowSelectionInterval( row, row);
				( ( ExTransferPropertyPanel)_propertyPanelBase).changeSelection( ( ExTransferObject)getValueAt( row, 0), false);
