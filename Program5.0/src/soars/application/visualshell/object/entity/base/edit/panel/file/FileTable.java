...
public class FileTable extends VariableTableBase implements DropTargetListener {
	 ...
	 * @param parent
	 */
	public FileTable(EntityBase entityBase, Map<String, PropertyPanelBase> propertyPanelBaseMap, PropertyPanelBase propertyPanelBase, Frame owner, Component parent) {
		super("file", entityBase, propertyPanelBaseMap, propertyPanelBase, 3, owner, parent);
	}
...
//		return LayerManager.get_instance().move_file( srcPath, destPath);
	...
	public boolean setup() {
		...
		if ( !_entityBase.is_multi()) {
			if ( !setup( _entityBase._objectMapMap.get( "file"), defaultTableModel))
				return false;
		}
	...
	private void update_initial_value(int row, File file) {
		...
		( ( FilePropertyPanel)_propertyPanelBase).changeSelection( ( FileObject)getValueAt( row, 0), true);
	}
	...
	@Override
	public void on_ok() {
		_entityBase._objectMapMap.get( "file").clear();

		for ( int i = 0; i < getRowCount(); ++i) {
			FileObject fileObject = ( FileObject)getValueAt( i, 0);
			_entityBase._objectMapMap.get( "file").put( fileObject._name, fileObject);
		}
	}
    ...
    DataFlavor[] dataFlavors = arg0.getCurrentDataFlavors();
		if ( dataFlavors[ 0].getHumanPresentableName().equals( DirectoryNodeTransferable._name)
			...
			try {
				...
				setRowSelectionInterval( row, row);
				( ( FilePropertyPanel)_propertyPanelBase).changeSelection( ( FileObject)getValueAt( row, 0), false);
