...
public class SubstitutionCommand extends NumberRule {
	...
	private void get_number_object_names1(String value, int kind, String prefix, Vector<String> numberObjectNames) {
		switch ( kind) {
			case 0:
				get_number_object_names( value, 2, numberObjectNames);
				break;
			case 1:
				get_number_object_names( value, 2, numberObjectNames);
				get_number_object_names( value, 2, new int[] { 1}, prefix, numberObjectNames);
				break;
			case 2:
				String[] elements = value.split( " ");
				if ( null == elements || 2 > elements.length)
	...
	protected boolean update_number_object_name(String name, String newName, String type, Role role) {
		...
		if ( subType._operator.equals( "")) {
			...
			String value0 = update_number_object_name1(
				values[ 0], subType._kind[ 0], name, newName, type);
			if ( null != value0)
				values[ 0] = value0;
	...
	protected String get_script(String value, Role role) {
		...
		if ( subType._operator.equals( "")) {
			String script = get_script1( main_value, subType._kind[ 0], false);
			if ( null == script)
				return "";
	...
	private String get_script1(String value, int kind, boolean bracket) {
		switch ( kind) {
			...
			case 2:
				...
				return get_expression_script1( expressionElements, elements[ 0], "", bracket);
			case 3:
	...
	public void get_used_expressions(TreeMap<String, Expression> expressionMap, TreeMap<String, Expression> usedExpressionMap) {
		...
		for ( int i = 0; i < expressionElements.length; ++i) {
			...
			Expression expression = expressionMap.get( expressionElements[ i].get_function());
			if ( null == expression)
				continue;
	...
	protected boolean get_on_immediate_value(String[] elements, GenericRule genericRule, Role role) {
		...
		// TODO typeを変更したらこれも変えること！
		genericRule._type = ResourceManager.get_instance().get( "generic.rule.command.set.variable.type");

		String valueType = update( genericRule);
		if ( null == valueType)
			return false;

		return get_on_immediate_value( elements[ 1], valueType.equals( "integer") ? "immediate integer" : "immediate real number", valueType, genericRule, role);
	}
	...
	protected boolean get_on_numeric_variable(String[] elements, GenericRule genericRule, Role role) {
		...
		if ( null == update( genericRule))
			return false;

		return get_on_numeric_variable( elements[ 1], genericRule, role);
	}
	 ...
	 * @return
	 */
	private String update(GenericRule genericRule) {
		// TODO 2014.2.19 genericRule._objects.get( 0)の数値変数が実数型か？整数型か？を調べて更新する
		EntityVariableRule entityVariableRule = ( EntityVariableRule)genericRule._objects.get( 0);

		// 数値変数のタイプを取得("integer"または"real number")
		String numberObjectType = entityVariableRule.get_number_object_type();
		if ( null == numberObjectType)
			return null;

		entityVariableRule._variableType = numberObjectType.equals( "integer") ? "integer number object" : "real number object";

		// TODO idを変更したらこれも変えること！
		genericRule._id = ResourceManager.get_instance().get( numberObjectType.equals( "integer") ? "generic.rule.command.set.integer.variable.id" : "generic.rule.command.set.real.number.variable.id");

		return numberObjectType;
	}
	...
	protected boolean get_on_expression(String[] elements, GenericRule genericRule, Role role) {
		...
		// TODO idを変更したらこれも変えること！
		genericRule._id = ResourceManager.get_instance().get( "generic.rule.command.numeric.expression.substitution.id");
	...
	protected Rule get_othersRule() {
		// TODO 2014.2.12
		return Rule.create( _kind, _column, ResourceManager.get_instance().get( "rule.type.command.others"), _value, _or);
	}
}
